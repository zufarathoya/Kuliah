...
std::vector<std::vector<double>> epochWeights;
...
void readIrisData(std::string filename) {
  ...
  while (std::getline(file, line)) {
    ...
    while (std::getline(ss, token, ',')) {
      if (token == "Iris-setosa" || token == "Iris-versicolor" ||
          token == "Iris-virginica") {
        continue;
      } else {
...
void storeWeights() { weightsHistory.emplace_back(w); }

void storeDeltaWeights() { deltaWeightsHistory.emplace_back(dw); }
...
void saveCombinedCSV(const std::string &filename) {
  ...
  file << "iteration,weight1,weight2,weight3,weight4,weight_bias,dw1,dw2,dw3,"
          "dw4,dw_bias,activation\n";
  for (int i = 0; i < weightsHistory.size() - 1; i++) {
    file << i + 1 << ",";
...
int main() {
  ...
  for (int epoch = 0; epoch < accuracies.size(); epoch++) {
    resultFile << epoch + 1 << "," << accuracies[epoch][0] << ","
               << meanErrors[epoch][0] << "\n";
  }
